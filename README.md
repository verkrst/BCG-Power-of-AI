# 🌾 ClientCo — Barley Yield & ESG Risk Dashboard
...
ClientCo sources barley from across France. Climate change is altering temperature, precipitation, and drought patterns — directly impacting barley yields. The company needs to:

1. **Quantify supply risk** at the department level under different climate futures
2. **Estimate financial & ESG impacts** (sourcing spend, CO₂e per ton)
...
├── prepare_predictions.py      # Data pipeline + model training + prediction generation
├── requirements.txt            # Python dependencies
├── departements.geojson        # France departments GeoJSON (mainland)
├── dataset/
│   ├── barley_yield_from_1982.csv       # Historical barley yield/area/production
│   ├── climate_data_from_1982.parquet   # CMIP6 climate data (historical + SSP projections)
│   └── predictions.csv                  # Pre-computed predictions (generated by prepare_predictions.py)
├── eda.ipynb                   # Final notebook
...
**Features** (growing season: March–July):

| Feature | Description |
|---------|-------------|
| `tmean_gs` | Mean temperature during growing season (°C) |
| `tmax_gs` | Mean daily max temperature (°C) |
| `prcp_gs` | Total precipitation (mm) |
| `hot_days` | Number of days with Tmax > 30°C |
...
Three headline metrics with evolution vs 2004–2014 baseline:
- **Predicted Total Production** (M tons)
- **Est. Sourcing Spend** (€M) — adjustable via price slider
- **Est. CO₂e per Ton** (kg) — based on fixed 2,500 kg CO₂e/ha emission factor

### 2. Interactive Choropleth Map
- Department-level **yield change (%)** vs baseline
- 5 discrete color bins (red → yellow → green)
- Hover tooltips: yield, baseline, production

### 3. Department Detail Panel
- Select any department for drill-down KPIs
- Yield change, predicted vs baseline yield, production, CO₂e per ton
...
- 4 quadrants with color-coded dots:
  - **Q1 — Core Crisis Zone**: High exposure + high risk → deep adaptation
  - **Q2 — Strategic Base**: High exposure + low risk → optimise & expand
  - **Q3 — Marginal High-Risk**: Low exposure + high risk → agile procurement
  - **Q4 — Long-Tail Safe**: Low exposure + low risk → monitor & comply
...
- **Barley Market Price** slider (€130–400/ton)
...
This trains the XGBoost model on historical data and generates `dataset/predictions.csv` (~9,300 rows covering all departments × years × scenarios).
...
- **Plotly** — interactive maps (Choroplethmapbox) & charts
